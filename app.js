...
function newInvoice(){
  ...
  document.getElementById('custGov').value = '';
...
async function saveInvoice(){
  ...
  try{
    const baseData = {
      ...
      governorate: document.getElementById('custGov').value.trim(),
...
function editInvoice(id){
  ...
  document.getElementById('custGov').value = inv.governorate || '';
  document.getElementById('custRegion').value = inv.region || '';
  document.getElementById('custState').value = inv.state || '';
  document.getElementById('deliveryType').value = inv.deliveryType || 'home';
  ...
  wrap.innerHTML = invoicesCache.map(inv=>{
    ...
    <div class="list-item">
      ...
      <div class="right-block">
        ...
        <div class="actions">
          <button class="icon-btn" title="ملصق شحن" onclick="App.quickLabel('${inv.id}')">🏷️</button>
...
// ---------------------------------------------------------------
// ملصقات الشحن (A6 × 4 في ورقة A4)
// ---------------------------------------------------------------
function renderLabelPickList(){
  const wrap = document.getElementById('labelsPickList');
  if(!wrap) return;
  if(invoicesCache.length === 0){ wrap.innerHTML = '<div class="empty">لا توجد فواتير بعد</div>'; return; }
  wrap.innerHTML = invoicesCache.map(inv=>`
    <label class="label-pick-item">
      <input type="checkbox" ${selectedLabelIds.has(inv.id)?'checked':''} onchange="App.toggleLabelSelect('${inv.id}')">
      <div>
        <div class="main">${inv.invNoStr||''} — ${inv.customerName || 'بدون اسم'}</div>
        <div class="sub">${fmtDateDisplay(inv.date)} · ${inv.state || ''} ${inv.governorate ? '- '+inv.governorate : ''}</div>
      </div>
    </label>
  `).join('');
}
...
function selectAllLabels(all){
  selectedLabelIds = all ? new Set(invoicesCache.map(i=>i.id)) : new Set();
  renderLabelPickList();
  renderLabelsPreview();
}

function deliveryLabelHTML(inv){
  const isOffice = inv.deliveryType === 'office';
  return `<div class="label-delivery ${isOffice?'office':'home'}">${isOffice ? '🏢 توصيل للمكتب' : '🏠 توصيل للمنزل'}</div>`;
}

function labelCardHTML(inv){
  const remaining = inv.remaining!=null ? inv.remaining : (inv.total||0);
  const status = inv.paymentStatus || 'unpaid';
  return `<div class="label-card">
    <div class="label-head">
      ${currentLogo ? `<img class="label-logo" src="${currentLogo}">` : `<div class="label-logo empty">شعار</div>`}
      <div>
        <div class="label-store-name">مبتكر MOBTAKER</div>
        <div class="label-contact-row">📞 90999815</div>
        <div class="label-contact-row">📸 mobtaker.om</div>
      </div>
    </div>
    <div class="label-body">
      <div class="label-row"><span class="lbl">العميل</span><span class="v">${inv.customerName || '-'}</span></div>
      <div class="label-row"><span class="lbl">الهاتف</span><span class="v" dir="ltr">${inv.custPhone || '-'}</span></div>
      <div class="label-row"><span class="lbl">المحافظة</span><span class="v">${inv.governorate || '-'}</span></div>
      <div class="label-row"><span class="lbl">الولاية</span><span class="v">${inv.state || '-'}</span></div>
      ${inv.region ? `<div class="label-row"><span class="lbl">المنطقة</span><span class="v">${inv.region}</span></div>` : ''}
      ${deliveryLabelHTML(inv)}
    </div>
    <div class="label-foot">
      <div class="label-ship"><span>تكلفة الشحن</span><b>${(inv.shipping||0).toFixed(3)} ر.ع</b></div>
      ${status !== 'paid' && remaining > 0 ? `<div class="label-cod"><span>يُحصَّل من العميل</span><b>${remaining.toFixed(3)} ر.ع</b></div>` : `<div class="label-cod"><span>حالة الدفع</span><b style="color:var(--green);">مدفوعة ✓</b></div>`}
      <div class="label-inv-no">${inv.invNoStr || ''}</div>
    </div>
  </div>`;
}
...
function renderLabelsPreview(){
  ...
  if(!sheet) return;
  const selected = invoicesCache.filter(inv => selectedLabelIds.has(inv.id));
  if(selected.length === 0){ sheet.innerHTML = '<div class="empty">اختر فاتورة أو أكثر من القائمة فوق لمعاينة الملصق</div>'; return; }
  sheet.innerHTML = selected.map(labelCardHTML).join('');
}

function printLabels(){
  if(selectedLabelIds.size === 0){ alert('اختر فاتورة واحدة على الأقل'); return; }
  showView('labels');
  setTimeout(()=> window.print(), 150);
}

function quickLabel(id){
  selectedLabelIds = new Set([id]);
  renderLabelPickList();
  renderLabelsPreview();
  showView('labels');
...
window.App = {
  ...
  toggleLabelSelect, selectAllLabels, printLabels, quickLabel
};
